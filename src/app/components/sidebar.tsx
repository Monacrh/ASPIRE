  ...
  return (
    <>
        ...
        <AnimatePresence>
          {!isOpen && (
            <motion.button
              ...
              className="p-3 bg-[#FFD93D] border-2 border-black shadow-[4px_4px_0_black] active:shadow-none active:translate-x-1 active:translate-y-1 transition-all"
            >
      ...
      <motion.aside
        ...
        `}
        style={!isMobile ? { boxShadow: '6px 0 0 rgba(0,0,0,0.1)' } : {}}
      >
        {/* HEADER */}
        ...
        <div className="p-5 border-b-4 border-black bg-[#fafafa] min-h-[100px] flex items-center justify-center shrink-0">
          <AnimatePresence mode="wait">
            {isOpen ? (
              <motion.button
                onClick={() => { if(isMobile) setIsOpen(false); router.push("/dashboard"); }}
                key="full-btn"
                variants={buttonClick}
                whileHover="hover"
                whileTap="tap"
                ...
                {isLoading ? (
                  ...
                  courses.map((item: Transcript) => (
                    ...
                    >
                      ...
                      >
                         {isDeleting === item._id?.toString() ? <span className="w-4 h-4 block rounded-full border-2 border-gray-400 border-t-black animate-spin" /> : <Trash2 className="w-4 h-4 text-red-600" />}
                      </button>
                    </motion.div>
                  ))
                )}
